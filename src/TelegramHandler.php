...
class TelegramHandler extends AbstractProcessingHandler
{
    ...
    {
        if ($template = config('laravel-logging-telegram.template')) {
    ...
    private function sendMessage(string $text): void
    {
        $httpQuery = http_build_query(array_merge(
            ...
            config('laravel-logging-telegram.options', [])
        ));
    ...
    private function getConfigValue($key, $defaultConfigKey = null): string
    {
        ...
        return config($defaultConfigKey ?: "laravel-logging-telegram.$key");
    }
}